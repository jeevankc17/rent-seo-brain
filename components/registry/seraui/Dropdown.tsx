...
const DropdownMenu = ({ children, trigger }: DropdownMenuProps) => {
  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      ...
      {isOpen && (
        <div
          className="origin-top-right absolute right-0 mt-2 w-48 rounded-lg shadow-lg bg-neutral-900 ring-1 ring-black ring-opacity-50 focus:outline-none z-50 animate-in fade-in-0 zoom-in-95 p-1"
          role="menu"
...
const DropdownMenuItem = ({ children, onClick, href }: DropdownMenuItemProps) => {
  const baseClass =
    "text-white font-semibold group flex items-center px-3 py-2 text-sm rounded-md hover:bg-neutral-800 transition-colors duration-150";
  if (href) {
    return (
...
// export components for reuse in header
